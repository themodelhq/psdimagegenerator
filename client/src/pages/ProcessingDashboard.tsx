...
interface DisplayJob {
  ...
  guestSizeId?: string;
  // S3 file keys received from WebSocket on completion (used for guest ZIP)
...
export default function ProcessingDashboard() {
  ...
  const { progress, isConnected } = useJobProgress({
    ...
    onComplete: (jobId, fileKeys) => {
      toast.success('Batch processing completed!');
      const done = (job: DisplayJob) =>
        job.id === jobId
          ? { ...job, status: 'completed', completedAt: new Date(), completedFileKeys: fileKeys }
          : job;
  ...
  const handleStartJob = async (job: DisplayJob) => {
    ...
    try {
      ...
      if (job.isGuest) {
        result = await startBatchMutation.mutateAsync({
          guestTemplateFileKey: job.guestTemplateFileKey,
          guestExcelFileKey: job.guestExcelFileKey,
          guestSizeId: job.guestSizeId,
        });
      } else {
        result = await startBatchMutation.mutateAsync({ jobId: job.id });
      }

      // The server returns the actual jobId it used for WebSocket broadcasts.
      // For guests the client-side pseudoId and server pseudoId differ (both Date.now()
      // but called at different times), so we must re-key the job to the server's ID.
      const serverJobId = result?.jobId;
      if (serverJobId && serverJobId !== job.id) {
        setGuestJobs(prev => prev.map(j =>
          j.id === job.id ? { ...j, id: serverJobId, status: 'processing' } : j
        ));
        setSelectedJobId(serverJobId);
        startedRef.current.add(serverJobId);
      }
  ...
  const handleCreateZip = async (job: DisplayJob) => {
    try {
      const input = job.isGuest && job.completedFileKeys?.length
        ? { fileKeys: job.completedFileKeys }
        : { jobId: job.id };
      const result = await createZipMutation.mutateAsync(input);
      if (result.zipUrl) {
                ...
                <CardContent className="space-y-5">
                  ...
                  <div className="flex gap-2">
                    ...
                    {selectedJob.status === 'completed' && (
                      <Button
                        onClick={() => handleCreateZip(selectedJob)}
