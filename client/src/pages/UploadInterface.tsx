...
import { isGuestMode, setPendingJob } from '@/hooks/useGuest';
    ...
    try {
      ...
      if (guest) {
        ...
        setPendingJob({
          isGuest: true,
          guestTemplateFileKey: state.guestTemplateFileKey!,
          guestExcelFileKey: state.excelFileKey!,
          guestSizeId: state.selectedSizeId,
          totalRows: state.rowCount,
        });
      } else {
        ...
        const authResult = await createBatchMutation.mutateAsync({
        ...
        // Store job ID so ProcessingDashboard can display it immediately
        if ((authResult as any).jobId) {
          setPendingJob({ isGuest: false, jobId: (authResult as any).jobId });
        }
      }
