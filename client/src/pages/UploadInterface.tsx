...
export default function UploadInterface() {
  const [state, setState] = useState<UploadState>({
    ...
    templateId: null, excelFileKey: null, excelFileUrl: null,
  ...
  const handlePsdChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file) return;
    if (!file.name.endsWith('.psd')) { setState(prev => ({ ...prev, psdError: 'Please select a .psd file' })); return; }
    setState(prev => ({ ...prev, psdFile: file, psdLoading: true, psdError: null }));
    try {
      const buffer = await file.arrayBuffer();
      const result = await uploadPsdMutation.mutateAsync({ fileName: file.name, fileBuffer: Buffer.from(buffer) });
      // Get the newly created template ID
      setState(prev => ({ ...prev, psdParsed: true, psdLoading: false, textLayers: result.textLayers as any || [] }));
  ...
  const handleExcelChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file) return;
    if (!file.name.match(/\.(xlsx|xls)$/)) { setState(prev => ({ ...prev, excelError: 'Please select an Excel file (.xlsx or .xls)' })); return; }
    setState(prev => ({ ...prev, excelFile: file, excelLoading: true, excelError: null }));
    try {
      const buffer = await file.arrayBuffer();
      const result = await parseExcelMutation.mutateAsync({ fileName: file.name, fileBuffer: Buffer.from(buffer) });
      setState(prev => ({ ...prev, excelParsed: true, excelLoading: false, excelFileKey: result.fileKey, excelFileUrl: result.fileUrl, excelHeaders: result.headers, rowCount: result.rowCount }));
  ...
  const handleStartBatch = async () => {
    if (!state.excelFileKey || !state.excelFileUrl) { toast.error('Upload Excel file first'); return; }
    try {
      // Get the latest template
      const templatesResp = await window.fetch('/api/trpc/template.list').then(r => r.json()).catch(() => null);
      const templates = templatesResp?.result?.data?.json || [];
      const latestTemplate = templates[templates.length - 1];
      if (!latestTemplate) { toast.error('No template found. Upload PSD first.'); return; }

      const jobResult = await createBatchMutation.mutateAsync({
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <Card className={state.psdParsed ? 'border-green-200' : ''}>
          ...
          <CardContent className="space-y-4">
            <label htmlFor="psd-input" className={`block border-2 border-dashed rounded-xl p-8 text-center cursor-pointer transition-all ${state.psdLoading ? 'opacity-50' : 'hover:border-purple-400 hover:bg-purple-50/50'} ${state.psdParsed ? 'border-green-300 bg-green-50' : 'border-slate-300'}`}>
              <input type="file" accept=".psd" onChange={handlePsdChange} disabled={state.psdLoading} className="hidden" id="psd-input" />
              {state.psdParsed ? (
                ...
                <>
                  ...
                  <p className="text-sm font-medium text-slate-700">{state.psdFile ? state.psdFile.name : 'Click to upload PSD'}</p>
                  <p className="text-xs text-slate-500 mt-1">Photoshop .psd file</p>
                </>
              )}
            </label>
            {state.psdLoading && <div className="space-y-2"><p className="text-sm text-slate-500">Parsing PSD layers…</p><Progress value={60} /></div>}
            {state.psdError && <Alert variant="destructive"><AlertCircle className="h-4 w-4" /><AlertDescription>{state.psdError}</AlertDescription></Alert>}
            {state.psdParsed && state.textLayers.length > 0 && (
              <div className="space-y-1.5">
                ...
                <div className="flex flex-wrap gap-1.5">
                  ...
                  {state.textLayers.length > 8 && <Badge variant="outline" className="text-xs">+{state.textLayers.length - 8} more</Badge>}
                </div>
              </div>
            )}
          </CardContent>
        ...
        <Card className={state.excelParsed ? 'border-green-200' : ''}>
          ...
          <CardContent className="space-y-4">
            <label htmlFor="excel-input" className={`block border-2 border-dashed rounded-xl p-8 text-center cursor-pointer transition-all ${state.excelLoading ? 'opacity-50' : 'hover:border-green-400 hover:bg-green-50/50'} ${state.excelParsed ? 'border-green-300 bg-green-50' : 'border-slate-300'}`}>
              <input type="file" accept=".xlsx,.xls" onChange={handleExcelChange} disabled={state.excelLoading} className="hidden" id="excel-input" />
              {state.excelParsed ? (
                <div className="flex flex-col items-center gap-2 text-green-700">
                  <CheckCircle className="w-8 h-8" />
                ...
                <>
                  <Upload className="w-8 h-8 mx-auto mb-2 text-slate-400" />
                  <p className="text-sm font-medium text-slate-700">{state.excelFile ? state.excelFile.name : 'Click to upload Excel'}</p>
                  <p className="text-xs text-slate-500 mt-1">.xlsx or .xls file</p>
                </>
              )}
            </label>
            {state.excelLoading && <div className="space-y-2"><p className="text-sm text-slate-500">Parsing Excel…</p><Progress value={60} /></div>}
            {state.excelError && <Alert variant="destructive"><AlertCircle className="h-4 w-4" /><AlertDescription>{state.excelError}</AlertDescription></Alert>}
            {state.excelParsed && (
              <div className="space-y-2">
                <p className="text-xs font-semibold text-slate-600">Columns detected:</p>
                <div className="flex flex-wrap gap-1.5">
                  {state.excelHeaders.map(h => (
                    <Badge key={h} variant="outline" className={`text-xs ${h.toLowerCase().includes('image') || h.toLowerCase().includes('sku') ? 'border-green-400 text-green-700' : ''}`}>{h}</Badge>
                  ))}
                </div>
              </div>
            )}
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              <div className="md:col-span-2 space-y-2">
                ...
                <Select value={state.selectedSizeId} onValueChange={v => setState(prev => ({ ...prev, selectedSizeId: v }))}>
                    ...
                    {DIAPER_SIZES.map(size => (
                      <SelectItem key={size.id} value={size.id}>
                        <div className="flex items-center gap-2">
                          <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-mono ${size.color}`}>{size.weightRange}</span>
            ...
            <div className="pt-2">
              ...
              <p className="text-xs text-slate-500 mt-2">Then go to the <strong>Batch</strong> tab to start processing {state.rowCount} products</p>
            </div>
          </CardContent>
        </Card>
      )}
          ...
          <CardContent className="pt-5">
            <div className="flex items-center gap-4 text-sm text-slate-600">
              <div className={`flex items-center gap-2 ${state.psdParsed ? 'text-green-700' : 'text-slate-500'}`}>
                {state.psdParsed ? <CheckCircle className="w-4 h-4" /> : <div className="w-4 h-4 rounded-full border-2 border-current" />}
                Upload PSD
              </div>
              <ChevronRight className="w-4 h-4 text-slate-300" />
              <div className={`flex items-center gap-2 ${state.excelParsed ? 'text-green-700' : 'text-slate-500'}`}>
                {state.excelParsed ? <CheckCircle className="w-4 h-4" /> : <div className="w-4 h-4 rounded-full border-2 border-current" />}
                Upload Excel
              </div>
              <ChevronRight className="w-4 h-4 text-slate-300" />
