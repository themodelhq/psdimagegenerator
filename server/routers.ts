...
export const appRouter = router({
  ...
  template: router({
    upload: guestOrUserProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        try {
          ...
          // Only persist to DB for real users (actorId is a numeric string for real users)
          if (ctx.user) {
    ...
    list: guestOrUserProcedure.query(async ({ ctx }) => {
      if (!ctx.user) return []; // guests have no persisted templates
  ...
  batch: router({
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        (async () => {
          ...
          try {
            ...
            notifyJobCompleted(pseudoJobId);
          } catch (error) {
    ...
    createZip: guestOrUserProcedure
      .input(z.object({ jobId: z.number() }))
      .mutation(async ({ input, ctx }) => {
        if (!ctx.user) throw new Error('Sign in to download ZIP');
        const job = await db.getProcessingJobById(input.jobId);
        if (!job || job.userId !== ctx.user.id) throw new Error('Job not found');
        ...
        const zipFileKey = `downloads/${ctx.actorId}/${input.jobId}-${Date.now()}.zip`;
