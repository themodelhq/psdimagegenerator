        ...
        (async () => {
          ...
          try {
            ...
            for (let i = 0; i < excelData.rows.length; i++) {
              ...
              try {
                // Extract count/packs/weight/type from Excel columns
                const rowCount = Number(
                  row['Pieces'] ?? row['pieces'] ?? row['count'] ?? row['Count'] ??
                  row['quantity'] ?? row['Quantity'] ?? row['diaper_count'] ?? ''
                ) || undefined;
                const rowPacks = Number(
                  row['packs'] ?? row['Packs'] ?? row['pack'] ?? row['Pack'] ??
                  row['num_packs'] ?? row['number_of_packs'] ?? 1
                ) || 1;
                // Use sheet weight/type columns to build the label (e.g. "16+ KG\nExtra Large")
                const rowWeightRange = String(row['weight range'] ?? row['weightRange'] ?? row['weight'] ?? '').trim();
                const rowType = String(row['Type '] ?? row['Type'] ?? row['type'] ?? '').trim();
                const rowWeightLabel = rowWeightRange && rowType ? rowWeightRange + '\n' + rowType : undefined;
                const result = await compositeProductIntoPSD({
                  psdPath: psdTempPath!,
                  productImageUrl: imgUrl || undefined,
                  sizeId,
                  count: isNaN(rowCount as number) ? undefined : rowCount,
                  packs: rowPacks,
                  weightLabel: rowWeightLabel,
                });
    ...
    generate: guestOrUserProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        try {
          ...
          const result = await compositeProductIntoPSD({ psdPath: psdTempPath, productImageUrl: input.productImageUrl, sizeId: input.sizeId, count: (input as any).count, packs: (input as any).packs ?? 1 });
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        try {
          ...
          for (const product of input.products) {
            try {
              const result = await compositeProductIntoPSD({ psdPath: psdTempPath, productImageUrl: product.image || undefined, sizeId: input.sizeId, count: (product as any).count, packs: 1 });
              if (result.success && result.outputBuffer) {
